...
/* Private variables ---------------------------------------------------------*/
static uint32_t CardType =  SDIO_STD_CAPACITY_SD_CARD_V1_1;	//?›¥????????????????????1.1§¿????
static uint32_t CSD_Tab[4], CID_Tab[4], RCA = 0;//?›¥CSD??DID???????????????
static uint8_t SDSTATUS_Tab[16]; //?›¥????????CSR???????
__IO uint32_t StopCondition = 0; //????????????????
__IO SD_Error TransferError = SD_OK; //????›¥?????????????????
__IO uint32_t TransferEnd = 0;	   //?????????????????????§Ø???????§Ö???
SD_CardInfo SDCardInfo;	  //????›¥?????????DSR????????
...
void SD_DMA_RxConfig(uint32_t *BufferDST, uint32_t BufferSize)
{
  ...
  DMA_ClearFlag(DMA2_FLAG_TC4 | DMA2_FLAG_TE4 | DMA2_FLAG_HT4 | DMA2_FLAG_GL4);//???DMA???¦Ë

  /*!< DMA2 Channel4 disable */
  DMA_Cmd(DMA2_Channel4, DISABLE);	//SDIO????????

  /*!< DMA2 Channel4 Config */
  DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)SDIO_FIFO_ADDRESS;  //????????fifo
  DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)BufferDST; //?????
  DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;	//?????????
  DMA_InitStructure.DMA_BufferSize = BufferSize / 4;  //????4?????????????¦Ë
  DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;//???????????????
  DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;	  //???›¥?????????
  DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Word;  //?????????§³????32¦Ë
  DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Word;	//?????????§³????32¦Ë
  DMA_InitStructure.DMA_Mode = DMA_Mode_Normal;			   //???????????????????adc??
  DMA_InitStructure.DMA_Priority = DMA_Priority_High;	 //??????????
  DMA_InitStructure.DMA_M2M = DMA_M2M_Disable;			 //?? ?›¥?????›¥????
  DMA_Init(DMA2_Channel4, &DMA_InitStructure);

  /*!< DMA2 Channel4 enable */			   //??????dma?§Ø??
...
void SD_DMA_TxConfig(uint32_t *BufferSRC, uint32_t BufferSize)
{
  ...
  DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralDST;//?????§Õ?????
...
/********************************************************************************************************/
   ...
   * or SD card 1.x 
	 */
  if (errorstatus == SD_OK)	//?????cmd55????sd?????????1.x,?????2.0
  {
  	/*???žD?????????sdio????????¦¶????????????*/
    ...
    {
      errorstatus = SD_INVALID_VOLTRANGE;	   /* SDIO?????card??????? */
      return(errorstatus);
    }
    ...
    {
      CardType = SDIO_HIGH_CAPACITY_SD_CARD; /* ??????????????sdsc????sdhc?? */
    }
...
/********************************************************************************************************/
  if (   (SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) 
       ...
       ||(SDIO_HIGH_CAPACITY_SD_CARD == CardType) )	 /* ??????2.0??? */
  {
...
SD_Error SD_EnableWideBusOperation(uint32_t WideMode)
{
  ...
  {														 
    ...
    else if (SDIO_BusWide_4b == WideMode)//4????????
    {
      errorstatus = SDEnWideBus(ENABLE);//???acmd6???????????????????????
      ...
      if (SD_OK == errorstatus)
      {
        ...
        SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;
        SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;
        SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_4b;	//?????????stm32??sdio?????? ???§Ý???????????sdio???????
        SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;
        SDIO_Init(&SDIO_InitStructure);
      }
    }
...
SD_Error SD_ReadMultiBlocks(uint8_t *readbuff, uint64_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  if (CardType == SDIO_HIGH_CAPACITY_SD_CARD)//sdhc????????????¦Ë?????512???
  {
    BlockSize = 512;
    ReadAddr /= 512;
  }

  /*!< Set Block Size for Card??cmd16,????sdsc??????????????????§³??????sdhc???????§³?512????????cmd16??? */
  SDIO_CmdInitStructure.SDIO_Argument = (uint32_t) BlockSize;
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SET_BLOCKLEN;
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;   //r1
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  ...
  SDIO_DataInitStructure.SDIO_DataTimeOut = SD_DATATIMEOUT;	 //??????????
  SDIO_DataInitStructure.SDIO_DataLength = NumberOfBlocks * BlockSize;	 //????????????????????????§Ö??????????????øA??(??SDIO_DCTRL)?????
  ...
  SDIO_ITConfig(SDIO_IT_DATAEND, ENABLE);  //???????????????§Ø? ??Data end (data counter, SDIDCOUNT, is zero) interrupt 
  SDIO_DMACmd(ENABLE); //???dma???
...
SD_Error SD_WriteBlock(uint8_t *writebuff, uint64_t WriteAddr, uint16_t BlockSize)
{
 ...
 /*********************************************************************************/
  ...
  SDIO_CmdInitStructure.SDIO_Argument = WriteAddr;	  //§Õ????
  ...
  SDIO_DataInitStructure.SDIO_DataLength = BlockSize;
  SDIO_DataInitStructure.SDIO_DataBlockSize = (uint32_t) 9 << 4;  //????????????SDIO_DataBlockSize_512b
  SDIO_DataInitStructure.SDIO_TransferDir = SDIO_TransferDir_ToCard;//§Õ?????
  SDIO_DataInitStructure.SDIO_TransferMode = SDIO_TransferMode_Block;
  SDIO_DataInitStructure.SDIO_DPSM = SDIO_DPSM_Enable;	 //???????????????
  SDIO_DataConfig(&SDIO_DataInitStructure);
...
#elif defined (SD_DMA_MODE)	//dma??
  SDIO_ITConfig(SDIO_IT_DATAEND, ENABLE);  //???????????§Ø?
  SD_DMA_TxConfig((uint32_t *)writebuff, BlockSize); //????dma????rx????
  SDIO_DMACmd(ENABLE);	 //	???sdio??dma????
#endif
...
SD_Error SD_WriteMultiBlocks(uint8_t *writebuff, uint64_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
 ...
 /*********************************************************************************/
  ...
  /*!< To improve performance *///  pre-erased??????§Õ?????????????????????
  SDIO_CmdInitStructure.SDIO_Argument = (uint32_t)NumberOfBlocks;  //????????§Õ???????
...
SD_Error SD_Erase(uint32_t startaddr, uint32_t endaddr)
{
  ...
  if (CardType == SDIO_HIGH_CAPACITY_SD_CARD)//sdhc???????? /512?????2.0§¿??page52
  {	//??sdhc?????????????????????512????sdsc???????????
    startaddr /= 512;	  
    endaddr /= 512;
  }
...
SD_Error SD_ProcessIRQSrc(void)
{
  ...
  {
    SDIO->ARG = 0x0;   //????????????
    SDIO->CMD = 0x44C;	  // ?????????? 0100 	01 	 	001100
						//						[7:6]  	[5:0]
						//				CPSMEN  WAITRESP CMDINDEX
						//		????????????	?????   cmd12 STOP_ TRANSMISSION						
  ...
  SDIO_ClearITPendingBit(SDIO_IT_DATAEND); //???§Ø?
  SDIO_ITConfig(SDIO_IT_DATAEND, DISABLE); //???sdio?§Ø????
  TransferEnd = 1;
  return(TransferError);
}
...
static SD_Error CmdError(void)
{
  ...
  /*!< Clear all the static flags */
  SDIO_ClearFlag(SDIO_STATIC_FLAGS);//?????????¦Ë
...
{		   
  ...
  SDIO->ICR = SDIO_STATIC_FLAGS;	//???§Ø???

  return (SD_Error)(SDIO->RESP1 &  SD_OCR_ERRORBITS);		//?§Ø????????¾œ¦¶
}
...
static SD_Error CmdResp6Error(uint8_t cmd, uint16_t *prca)
{
  ...
  {
    *prca = (uint16_t) (response_r1 >> 16);//????16¦Ë???????????????rca
    return(errorstatus);
  }
...
static SD_Error SDEnWideBus(FunctionalState NewState)
{
  ...
  /*!< Get SCR Register */
  errorstatus = FindSCR(RCA, scr);//???scr??????????scr??????

  if (errorstatus != SD_OK)		  //degug,crc????scr??????????
  {
    return(errorstatus);
  }
  ...
  if (NewState == ENABLE)
  {
    ...
    if ((scr[1] & SD_WIDE_BUS_SUPPORT) != SD_ALLZERO)  //?§Ø????????4¦Ë???
    {
...
{ 
  ...
  SDIO_CmdInitStructure.SDIO_Argument = (uint32_t)8;	 //???§³?????sdhc????????????§³??	//?????8
  ...
  /*???????????????*/
  SDIO_DataInitStructure.SDIO_DataTimeOut = SD_DATATIMEOUT;
  SDIO_DataInitStructure.SDIO_DataLength = 8;  //8byte,64¦Ë
